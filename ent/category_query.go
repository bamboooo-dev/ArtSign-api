...
func (cq *CategoryQuery) QueryWorks() *WorkQuery {
...
func (cq *CategoryQuery) WithWorks(opts ...func(*WorkQuery)) *CategoryQuery {
	query := &WorkQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withWorks; query != nil {
		...
		query.Where(predicate.Work(func(s *sql.Selector) {
			s.Where(sql.InValues(category.WorksColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.category_works
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_works" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_works" returned %v for node %v`, *fk, n.ID)
			}
