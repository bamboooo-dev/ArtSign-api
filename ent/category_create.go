...
func (cc *CategoryCreate) AddWorkIDs(ids ...int) *CategoryCreate {
	cc.mutation.AddWorkIDs(ids...)
	return cc
}

// AddWorks adds the "works" edges to the Work entity.
func (cc *CategoryCreate) AddWorks(w ...*Work) *CategoryCreate {
	ids := make([]int, len(w))
	for i := range w {
		ids[i] = w[i].ID
	}
	return cc.AddWorkIDs(ids...)
}
...
func (cc *CategoryCreate) createSpec() (*Category, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.WorksIDs(); len(nodes) > 0 {
