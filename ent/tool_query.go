...
func (tq *ToolQuery) QueryWorks() *WorkQuery {
...
func (tq *ToolQuery) WithWorks(opts ...func(*WorkQuery)) *ToolQuery {
	query := &WorkQuery{config: tq.config}
...
func (tq *ToolQuery) sqlAll(ctx context.Context) ([]*Tool, error) {
	...
	if query := tq.withWorks; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Tool, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Works = []*Work{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Tool)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tool.WorksPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "works": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "works" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Works = append(nodes[i].Edges.Works, n)
			}
		}
	}
