...
func (cq *CommentQuery) QueryWork() *WorkQuery {
	query := &WorkQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithWork(opts ...func(*WorkQuery)) *CommentQuery {
	query := &WorkQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context) ([]*Comment, error) {
	...
	if query := cq.withWork; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Comment)
		for i := range nodes {
			if nodes[i].work_comments == nil {
				continue
			}
			fk := *nodes[i].work_comments
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "work_comments" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withChildren; query != nil {
		...
		for _, n := range neighbors {
			fk := n.comment_parent
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "comment_parent" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "comment_parent" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := cq.withParent; query != nil {
		...
		nodeids := make(map[int][]*Comment)
		for i := range nodes {
			if nodes[i].comment_parent == nil {
				continue
			}
			fk := *nodes[i].comment_parent
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "comment_parent" returned %v`, n.ID)
			}
			for i := range nodes {
