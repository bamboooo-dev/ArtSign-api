...
func (iq *ImageQuery) QueryWork() *WorkQuery {
	query := &WorkQuery{config: iq.config}
...
func (iq *ImageQuery) WithWork(opts ...func(*WorkQuery)) *ImageQuery {
	query := &WorkQuery{config: iq.config}
...
func (iq *ImageQuery) sqlAll(ctx context.Context) ([]*Image, error) {
	...
	if query := iq.withWork; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Image)
		for i := range nodes {
			if nodes[i].work_images == nil {
				continue
			}
			fk := *nodes[i].work_images
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "work_images" returned %v`, n.ID)
			}
			for i := range nodes {
