...
func (uq *UserQuery) QueryLikes() *WorkQuery {
	query := &WorkQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryLikeComments chains the current query on the "like_comments" edge.
func (uq *UserQuery) QueryLikeComments() *CommentQuery {
	query := &CommentQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryFollowees() *UserQuery {
	query := &UserQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, user.FolloweesTable, user.FolloweesPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLikeComments: uq.withLikeComments.Clone(),
...
func (uq *UserQuery) WithLikes(opts ...func(*WorkQuery)) *UserQuery {
	query := &WorkQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithLikeComments tells the query-builder to eager-load the nodes that are connected to
// the "like_comments" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLikeComments(opts ...func(*CommentQuery)) *UserQuery {
	query := &CommentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withLikeComments = query
	return uq
}
...
func (uq *UserQuery) WithFollowees(opts ...func(*UserQuery)) *UserQuery {
	query := &UserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withWorks; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Works = []*Work{}
		}
		query.withFKs = true
		query.Where(predicate.Work(func(s *sql.Selector) {
			s.Where(sql.InValues(user.WorksColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_works
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_works" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_works" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Works = append(node.Edges.Works, n)
		}
	}
	...
	if query := uq.withPortfolios; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_portfolios" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_portfolios" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withLikeComments; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.LikeComments = []*Comment{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*User)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.LikeCommentsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "like_comments": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "like_comments" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.LikeComments = append(nodes[i].Edges.LikeComments, n)
			}
		}
	}
	...
	if query := uq.withFollowees; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.FolloweesPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "followees": %w`, err)
		}
		query.Where(user.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "followees" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Followees = append(nodes[i].Edges.Followees, n)
			}
		}
	}
