...
func (e ImageEdges) WorkOrErr() (*Work, error) {
	if e.loadedTypes[0] {
		if e.Work == nil {
...
func (*Image) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case image.ForeignKeys[0]: // work_images
			values[i] = new(sql.NullInt64)
		default:
	...
	for j := range columns {
		...
		case image.ForeignKeys[0]:
			if value, ok := values[j].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field work_images", value)
			} else if value.Valid {
				i.work_images = new(int)
				*i.work_images = int(value.Int64)
			}
		}
	}
	return nil
...
// QueryWork queries the "work" edge of the Image entity.
func (i *Image) QueryWork() *WorkQuery {
	return (&ImageClient{config: i.config}).QueryWork(i)
}
